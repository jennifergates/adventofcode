...
foreach ($draw in $drawings.split()){
	...
	if ($startat6 -gt 5) {
		...
		foreach ($b in 1..$boardcount) {
			if ($b -notin $winningboards){
				write-host "Board: " $b
				foreach ($i in 1..5){
					#write-host "Row: " $i
					$markedrow =$markedboards | where-object {$_.board -eq $b -and $_.row -eq $i -and $_.marked -eq 1} 
					if (($markedrow | measure-object).count -eq 5 -and ($markedrow | where-object { $_.integer -eq $draw})){
