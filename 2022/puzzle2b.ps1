...
foreach ($round in $n){
	...
	write-host "Opp: $opp and End: $end"

	if ($end -eq '2') { $roundscore = [int]$opp + 3; write-host "Draw $roundscore";}
	if ($end -eq '1') {
		switch ($opp)
		{
			'1' {$roundscore = 0 + 3; write-host "Lost $roundscore";}
			'2' {$roundscore = 0 + 1; write-host "Lost $roundscore";}
			'3' {$roundscore = 0 + 2; write-host "Lost $roundscore";}
		}
	}
	if ($end -eq '3') {
		switch ($opp)
		{
			'1' {$roundscore = 6 + 2; write-host "Win $roundscore";}
			'2' {$roundscore = 6 + 3; write-host "Win $roundscore";}
			'3' {$roundscore = 6 + 1; write-host "Win $roundscore";}
		}	
	}
<#
	switch ($round)
